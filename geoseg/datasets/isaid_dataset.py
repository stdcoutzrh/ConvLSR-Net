...
ORIGIN_IMG_SIZE = (896, 896)
INPUT_IMG_SIZE = (896, 896)
TEST_IMG_SIZE = (896, 896)
...
def train_aug(img, mask):
    crop_aug = Compose([RandomScale(scale_list=[0.75, 1.0, 1.25, 1.5], mode='value'),
                        SmartCropV1(crop_size=896, max_ratio=0.75,
                                    ignore_index=255, nopad=False)])
...
def val_aug(img, mask):
    #crop_aug = Compose([PadImage(size=(1024,1024), ignore_index = 255)])
    #img, mask = crop_aug(img, mask)
    img, mask = np.array(img), np.array(mask)
...
class ISAIDDataset(Dataset):
    def __init__(self, data_root='/home/zrh/datasets/semantic/isaid/train', mode='val', img_dir='images', mask_dir='masks',
